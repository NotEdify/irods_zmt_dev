      ...
      if (totalMeta > AVUThresholdBeforeWarningForNoSpecificQuery) {
        ...
        result.message = [
          <span key="unusedMeta1">
            ...
            this check to run, please add the specific query:{" "}
            <Chip label={specificQuery} /> either manually, or by using this{" "}
            <a
              target="_blank"
              rel="noreferrer"
    ...
    if (specificWarning) {
      result.message.push(
        <span key="unusedMeta3">
          ...
          query: <Chip label={specificQuery} /> either manually, or by using
          this{" "}
          <a
            rel="noreferrer"
            target="_blank"
