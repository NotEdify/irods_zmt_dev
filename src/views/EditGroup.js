...
export const EditGroup = (props) => {
  ...
  const loadCurrentGroupInfo = useCallback(() => {
    ...
    fetch(`${httpApiLocation}/query?${params.toString()}`, {
      ...
      .then((res) => res.json())
      .then((data) => {
        setUsersInGroup(data.rows);
        setLoading(false);
      });
  }, [auth, httpApiLocation]);
  ...
  return (
    <Fragment>
      ...
      <Link to="/groups" className={classes.link_button}>
        <Button>
          <ArrowBackIcon />
        </Button>
      </Link>
